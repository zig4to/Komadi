...
**Data model:** a single Supabase table, `songs` (`supabase/schema.sql` + `supabase/migrations/`, applied by hand in the Supabase SQL Editor — there's no migration runner). `genre` and `era` are fixed enums (`GENRES`, `ERAS` in `src/lib/constants.ts`, typed as unions in `src/types/song.ts`); `mood` and `origin` are free-text and user-extensible — `DEFAULT_MOODS`/`DEFAULT_ORIGINS` are only seed suggestion lists, not constraints, and `SongForm` lets the user type a new value that then shows up in future pickers/filters. `copy_count` drives the "Popularno" sort (see below). A second table, `author_images` (`author` text PK, `image_url`), holds one optional photo per author, settable from the small camera-icon button on any `SongCard` (`onSetAuthorImage`) and read into `authorImages` state in `Dashboard.tsx` for use as background art on cards/highlights. RLS policies allow public read/write with just the anon key — there is no auth, by design (personal single-user app; see "Varnostna opomba" in README.md before changing this).

**"Popularno" popularity is driven by chord-button clicks, not song-title clicks.** Earlier versions copied the title to the clipboard on any click anywhere on a card and counted *that* towards `copy_count` — this was removed. Now `copy_count` only increments when the "UG Tabs" or "PDF akordi" button is clicked (`handleChordsClick` in `Dashboard.tsx`, threaded down as `onChordsClick` through `SongCard`, `ChordsButtons`, and `FeaturedArtists`/`SongRow`). Cards no longer copy anything to the clipboard at all.

**Component map:**
- `SongCard.tsx` — one song: favorite/edit/delete, "Podobno" (AI similar-songs panel), clickable author name (sets the search filter to that author), and up to three chord affordances depending on what's set: an "UG Tabs" pill (`chords_source_url`, external link), a "PDF akordi" button (`chords_url` ending in `.pdf`, opens `PdfViewer` full-screen via a portal), or a plain "Akordi" external link fallback (`chords_url` set but not a PDF and no UG source).
- `ChordsButtons.tsx` — a shortened ("UG"/"PDF" instead of "UG Tabs"/"PDF akordi") version of the same two buttons + `PdfViewer` portal, reused on the compact card/row components that don't have room for full labels: `CompactCard`/`CompactRow` (Novo/Popularno tabs), `RecentGroupSongRow` (Novo-po-… groups), and `SongRow` in `FeaturedArtists.tsx` (Predstavljeno). Deliberately duplicates `SongCard.tsx`'s modal logic instead of sharing it, so each stays self-contained. Takes an optional `stacked` prop to lay the two buttons out vertically (used on `CompactRow` and `FeaturedArtists`' `SongRow`, where they sit to the right of the title instead of below it).
